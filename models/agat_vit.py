...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., patch_drop=0.):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        ...
        own_attn = torch.sum(attn, dim=1) # B N N
        own_attn = torch.sum(own_attn, dim=-2) # B N
        group_num = own_attn.shape[-1]
        k_num = int(group_num * (1.0 - self.patch_drop)) - 1
        _, rank_indices = torch.topk(own_attn[:,1:], k=k_num, dim=-1) # B K
        rank_indices = rank_indices + 1
        zeros_indices = torch.zeros([rank_indices.shape[0],1], dtype=rank_indices.dtype).cuda()
        rank_indices = torch.cat([zeros_indices, rank_indices],dim=1)
        rank_indices = rank_indices.unsqueeze(-1).repeat(1,1,self.dim)


        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = torch.gather(x, dim=1, index=rank_indices)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, rank_indices


class Block(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, patch_drop=0.):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, patch_drop=patch_drop)
    ...
    def forward(self, x):
        av, rindices = self.attn(self.norm1(x))
        x = torch.gather(x, dim=1, index=rindices) + self.drop_path(av)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', patch_drop=0.):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, patch_drop=patch_drop)
    ...
    def forward_features(self, x, kept_indices=None):
        ...
        if kept_indices is not None:
            # drop_patch_rate = 0.4
            # B, N, C = x.shape
            # indices = np.random.choice(N-1, int((N-1)*(1-drop_patch_rate)), replace=False) + 1
            # x = x[:, [0]+indices.tolist()]
            kept_indices = kept_indices.unsqueeze(-1).repeat(1,1,self.embed_dim)
            x = torch.gather(x, dim=1, index=kept_indices)
    ...
    def forward(self, x, kept_indices=None):
        x = self.forward_features(x, kept_indices=kept_indices)
